...
Phases:
  ...
  C  Narrative: before/after deltas cited in paper / pitch material.
  +  Latency and size: cross-phase, hardware-disclosed.

Exit 0 = all retained hard claims match the current checkpoint/reports.
Exit 1 = at least one retained hard claim fails. Historical claims that the
project has explicitly corrected are checked as measured limitations rather
than preserved as expected successes.
...
# ──────────────────────────────────────────────────────────────────────
# Claim registry — tolerances + expected values for the retained Path A claim set.
...
PHASE_A_EXPECTED = {
    ...
    'overall_fpr':       ('abs', 0.342,   0.010, True),
}
...
# (25, 26) Latency targets: deployment gate + historical reference.
...
# (24) γ ablation — both endpoints are now re-runnable checkpoints.
GAMMA_0_5_AUC = 0.804
GAMMA_3_0_AUC_RETRAIN = 0.791
...
def phase_a(device: torch.device) -> Dict:
    ...
    rows.append(check('Overall FPR @ 0.50', metrics['overall_fpr'],
                      PHASE_A_EXPECTED['overall_fpr'][1],
                      PHASE_A_EXPECTED['overall_fpr'][2],
                      hard=PHASE_A_EXPECTED['overall_fpr'][3],
                      detail='measured safe->unsafe leakage at threshold 0.50'))
    ...
    if os.path.exists(gamma3_path):
        try:
            ...
            # at thr=0.5. The current γ-collapse claim is that this value
            # drops to near zero at γ=3.0 while threat TPR rises; the earlier
            # AUC-improvement snapshot did not reproduce.
            import numpy as _np
            ...
            rows.append(check('γ=3.0 AUC (measured)', g3_auc,
                              GAMMA_3_0_AUC_RETRAIN, 0.01, hard=True,
            ...
            rows.append(check('γ=3.0 AUC (measured)', None,
                              GAMMA_3_0_AUC_RETRAIN, 0.01,
                              hard=False, kind='abs',
                              detail=f"load/inference failed: {type(e).__name__}: {e}"))
    else:
        rows.append(check('γ=3.0 AUC retrain',
                          None, GAMMA_3_0_AUC_RETRAIN, None,
                          hard=False, kind='info',
                          detail=f'{gamma3_path} not on disk — '
                                 f'γ=3.0 correction cannot be rechecked'))
...
def phase_b(skip: bool = False) -> List[Dict]:
    ...
    for site, ambient_dir, threat_dir in SITES:
        ...
        if blob is None:
            rows.append(check(f"B:{site} FP point ≤ 5%", None, 0.05, 0.0,
            ...
            rows.append(check(f"B:{site} Threat recall point", None, 0.789, 0.002,
                              hard=not skip, kind='abs',
                              detail='awaits phase_b JSON'))
            rows.append(check(f"B:{site} Speech-FP proxy", None, 0.169, 0.002,
                              hard=not skip, kind='abs',
                              detail='awaits phase_b JSON'))
            ...
            rows.append(check(f"B:{site} Silence gating", None, True, None,
                              ...
                              detail='awaits phase_b JSON'))
            continue
        rows.append(check(f"B:{site} FP point ≤ 5%", blob.get('fp_rate'), 0.05, 0.0,
                          hard=True, kind='lte',
                          detail=(f"point estimate only; n={blob.get('held_out_wavs','?')} "
                                  "held-out wavs, Wilson CI reported in RESULTS.md")))
        rows.append(check(f"B:{site} Threat recall point",
                          blob.get('threat_recall'), 0.789, 0.002,
                          hard=True, kind='abs',
                          detail=f"limited result; threats={blob.get('threat_dir','?')}"))
        rows.append(check(f"B:{site} Speech-FP proxy",
                          blob.get('speech_fp_post'), 0.169, 0.002,
                          hard=True, kind='abs',
                          detail='universal as_speech proxy, not CI-supported deployment evidence'))
...
def phase_c(phase_a: Dict, phase_b_rows: List[Dict]) -> List[Dict]:
    print()
    print("=" * 80)
    print("PHASE C  Narrative (paper / release-note deltas)")
    print("=" * 80)
    rows: List[Dict] = []

    # 21 — post-fine-tune speech FP proxy (pre leg from Phase A; post leg
    # from Phase B blob, if present).
    ...
    rows.append(check('C:Speech FP post proxy',
                      sfp_post, 0.169, 0.002, hard=blob is not None,
                      kind='abs',
                      detail=('measured after metro fine-tune; not CI-supported' if blob is not None
                              else 'awaits Phase B')))

    # 22 — post-fine-tune threat recall is a limited/failed deployment result.
    tr_post = blob.get('threat_recall') if blob else None
    rows.append(check('C:Post-FT recall point', tr_post, 0.789, 0.002,
                      hard=blob is not None,
                      detail=('measured after metro fine-tune; below target' if blob is not None
                              else 'awaits Phase B')))
    ...
    # checkpoint is on disk, else keep a soft missing-artifact row.
    ...
    if phase_a_gamma3 is not None and phase_a_gamma3['measured'] is not None:
        ...
        rows.append(check('C:γ=3.0 AUC retrain',
                          None, GAMMA_3_0_AUC_RETRAIN, None,
                          hard=False, kind='info',
                          detail='models/safecommute_v2_gamma3.pth missing'))
...
def latency_and_size() -> List[Dict]:
    ...
    # ── FP32 ONNX (the historical "~12 ms on Ryzen 5, 1T" path) ─────────
    ...
    if os.path.exists(onnx_path):
        try:
            ...
            rows.append(check('ONNX FP32 mean ~12 ms (historical ref)',
                              onnx_stats['mean'], HISTORICAL_LATENCY_MS, 3.0,
                              hard=hw_hard, kind='lte',
            ...
            rows.append(check('ONNX FP32 mean ~12 ms (historical ref)',
                              None, HISTORICAL_LATENCY_MS, None,
                              hard=False, kind='lte',
...
GAMMA_HISTORICAL = {
    # RESULTS.md "Experiment Summary" + the paper package ablation discussion.
...
def main():
    ...
    print()
    print("PCEN reconstruction audit - performance-claim verification")
    print("=" * 80)
